...
3. **Local AI** is selected by default. The Mac app starts the bundled Ollama engine, using the downloaded `gemma3:4b` model without an API key. **Local AI · Settings** shows its status. Optionally select OpenAI and connect an API key to use GPT-5.4.
...
Local generation is available in the Mac app or a local preview. A hosted website cannot reach the model on your Mac through its server. This version is being delivered locally.
