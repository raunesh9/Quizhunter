...
import {MAX_SLIDES_PER_FILE,MAX_FILE_MB} from './limits';
...
async function parsePDF(file:File,onProgress:(s:string)=>void):Promise<Deck>{
 ...
 try{if(pdf.numPages>MAX_SLIDES_PER_FILE)throw Error(`Please split the PDF into files of ${MAX_SLIDES_PER_FILE} pages or fewer.`);const slides:Slide[]=[];for(let n=1;n<=pdf.numPages;n++){onProgress(`Reading page ${n} of ${pdf.numPages}…`);const page=await pdf.getPage(n);const content=await page.getTextContent();const text=content.items.map(x=>'str' in x?x.str+('hasEOL' in x&&x.hasEOL?'\n':' '):'').join('');const v=page.getViewport({scale:1});const viewport=page.getViewport({scale:Math.min(1.5,1400/Math.max(v.width,v.height))});const canvas=document.createElement('canvas');canvas.width=Math.ceil(viewport.width);canvas.height=Math.ceil(viewport.height);const ctx=canvas.getContext('2d');if(!ctx)throw Error('Your browser could not render this PDF.');await page.render({canvasContext:ctx,canvas,viewport}).promise;slides.push({number:n,title:text.trim().split('\n')[0].slice(0,160)||`Page ${n}`,text:text.slice(0,50000),notes:'',images:[canvas.toDataURL('image/jpeg',.8)]});page.cleanup();canvas.width=0;canvas.height=0;}return {name:file.name.replace(/\.pdf$/i,''),slides,warnings:['PDF pages are treated as slides. Speaker notes are included only if they appear on the PDF pages.']}}finally{await task.destroy()}
}
